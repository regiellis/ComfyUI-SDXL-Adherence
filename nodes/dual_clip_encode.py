...
def _ramp_points(total_steps: int | None):
    ...
    def _norm(r):
        out = []
        for a,b,s in r:
            out.append((_round4(max(0.0, min(1.0, a))),
                        _round4(max(0.0, min(1.0, b))),
                        float(s)))
        return out
...
class SDXLDualClipEncode:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "late_text":  ("STRING", {"multiline": True, "default": "", "tooltip": "Aesthetics / long-tail details."}),
                "neg_text":   ("STRING", {"multiline": True, "default": "", "tooltip": "Negative prompt."}),
                "essentials_text": ("STRING", {"multiline": False, "default": "", "tooltip": "Keywords to softly reinforce."}),
                "early_late_mix": ("FLOAT", {"default": 0.4, "min": 0.0, "max": 1.0, "tooltip": "Blend for late_text in custom/short mode."}),
                "essentials_lock": ("FLOAT", {"default": 0.35, "min": 0.0, "max": 1.0, "tooltip": "Extra weight for essentials in custom/short mode."}),
                "clip_skip_openclip": ("INT", {"default": 1, "min": 0, "max": 2, "tooltip": "OpenCLIP skip (global)."}),
                "clip_skip_clipL":    ("INT", {"default": 0, "min": 0, "max": 2, "tooltip": "CLIP-L skip (local)."}),
                "mode": (["auto", "core_parity", "custom_only"], {
                    "default": "auto",
            ...
            "optional": {
                ...
                "width":        ("INT",   {"default": 1024, "min": 64, "max": 8192, "tooltip": "For res-aware heuristics (optional)."}),
                "height":       ("INT",   {"default": 1024, "min": 64, "max": 8192, "tooltip": "For res-aware heuristics (optional)."}),
                "total_steps":  ("INT",   {"default": 0, "min": 0, "max": 200, "tooltip": "Optional: sampler total steps for adaptive ramps."}),
                "cfg":          ("FLOAT", {"default": 0.0, "min": 0.0, "max": 30.0, "tooltip": "Optional: CFG for adaptive cap."}),
                "cfg_rescale":  ("FLOAT", {"default": 0.0, "min": 0.0, "max": 2.0, "tooltip": "Optional: warn if extreme (can cause late flips)."}),
            },
        }
    ...
    def encode(
        ...
        HEAD_RAMP, TAIL_RAMP, ESSENTIALS_RAMP = _ramp_points(total_steps if total_steps > 0 else None)
        ...
        if mode == "core_parity" or (mode == "auto" and long_prompt):
            ...
            if mode == "auto":
                ...
                if head_tok is not None and extra_weight_budget > 0.005:
                    h_cond, h_pooled = _encode_text_tokens(clip, head_tok, clip_skip_openclip, clip_skip_clipL)
                ...
                if tail_tok is not None and extra_weight_budget > 0.005:
                    t_cond, t_pooled = _encode_text_tokens(clip, tail_tok, clip_skip_openclip, clip_skip_clipL)
                ...
                if (essentials_text or "").strip() and not very_long and extra_weight_budget > 0.005:
                    ...
                    if eff_w > 0.01:
                        e_list = _encode_as_list(clip, essentials_text, clip_skip_openclip, clip_skip_clipL, weight=1.0)
            ...
            # Optional debug
            if _debug_enabled():
                print(f"[Adherence] mode=auto long={long_prompt} tokens={total_tokens} cap={_round4(extra_cap)} head={head_n} tail={tail_n} entries={len(pos)}")
            ...
            if _debug_enabled():
                print(f"[Adherence] mode={mode} long={long_prompt} tokens={total_tokens} entries={len(pos)}")
            ...
            for i, entry in enumerate(lst):
                ...
                if meta.get("pooled_output", None) is None:
                    if cond.ndim == 3:
                        ...
                        raise ValueError(f"{name}[{i}] has no pooled_output and cond shape {tuple(cond.shape)}")
                meta["weight"] = _round4(meta.get("weight", 1.0))
                # Clamp meta timesteps into [0,1]
                if "timestep_start" in meta:
                    meta["timestep_start"] = _round4(max(0.0, min(1.0, float(meta["timestep_start"]))))
